...
def worker(miner_id):
    ...
    while True:
        try:
            job, request_latency = send_miner_request(base_config, miner_id)
            if job is not None:
...
def main_loop():
    ...
    try:
        ...
        if miner_id is None or not miner_id.startswith("0x"):
            logging.warning("Warning: Configure your ETH address correctly in the .env file. Current value:", miner_id)
        configure_logging(base_config, miner_id)
